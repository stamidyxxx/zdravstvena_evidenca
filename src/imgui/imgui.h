...
namespace ImGui
{
    ...
    IMGUI_API void          Spacing(int repeat = 1);                                                      // add vertical spacing.
    ...
    IMGUI_API void          TextWrappedCentered(const char* text);
    ...
    IMGUI_API bool          TextSelectable(const char* text);
    ...
    IMGUI_API bool          InputText(const char* label, char* buf, ImVec2 size = ImVec2(0, 0), size_t buf_size = 0, ImGuiInputTextFlags flags = 0, ImGuiInputTextCallback callback = NULL, void* user_data = NULL);
    ...
    IMGUI_API bool          Selectable(const char* label, bool selected = false, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0), bool wrapped = false); // "bool selected" carry the selection state (read-only). Selectable() is clicked is returns true so you can modify your selection state. size.x==0.0: use remaining width, size.x>0.0: specify width. size.y==0.0: use label height, size.y>0.0: specify height
    IMGUI_API bool          Selectable(const char* label, bool* p_selected, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0), bool wrapped = false);      // "bool* p_selected" point to the selection state (read-write), as a convenient helper.
